...
def load_trained_model(model_location):
    ...
    loaded_model = load_model(model_location)
    return loaded_model, None  # TensorFlow 2.x does not use sessions
...
def burn_area(output_mask, resolution, forest_type):
  ...
  print("Which equivalent to:", "{:.4e}".format(equal_days), " days of India's  daily electricity power emission \n")
